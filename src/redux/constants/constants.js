import firebase from 'firebase';

const SIGNUP_FAILURE = 'SIGNUP_FAILURE';
const SIGNUP_SUCCESS =  'SIGNUP_SUCCESS';
const RESETPASSWORDLINK = 'RESETPASSWORDLINK';
const RESETPASSWORDLINKERROR = 'RESETPASSWORDLINKERROR';
const PASSWORDCONFIRM = 'PASSWORDCONFIRM';
const PASSWORDCONFIRMERROR = 'PASSWORDCONFIRMERROR';
const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
const LOGIN_FAILURE = 'LOGIN_FAILURE';
const SOCIAL_FAILURE = 'SOCIAL_FAILURE';
const SOCIAL_SUCCESS =  'SOCIAL_SUCCESS';
const SEARCH_SUCCESS='SEARCH_SUCCESS';
const SEARCH_FAILURE='SEARCH_FAILURE';
const FOLLOW_SUCCESS = 'FOLLOW_SUCCESS';
const FOLLOW_FAILURE = 'FOLLOW_FAILURE';
const FOLLOWERS_SUCCESS = 'FOLLOWERS_SUCCESS';
const FOLLOWERS_FAILURE = 'FOLLOWERS_FAILURE';
const FOLLOWING_SUCCESS = 'FOLLOWING_SUCCESS';
const FOLLOWING_FAILURE = 'FOLLOWING_FAILURE';
const UNFOLLOW_SUCCESS = 'UNFOLLOW_SUCCESS';
const UNFOLLOW_FAILURE = 'UNFOLLOW_FAILURE';
const LIKE_SUCCESS='LIKE_SUCCESS';
const LIKE_FAILURE='LIKE_FAILURE';
const PROMPT_ACTION='PROMPT_ACTION';
const PROMPT_CLOSE='PROMPT_CLOSE';
const CREATE_COMMENT='CREATE_COMMENT';
const CREATE_REPLY='CREATE_REPLY';
const READ_COMMENTS='READ_COMMENTS';
const CREATE_COMMENT_FAILURE='CREATE_COMMENT_FAILURE';
const READ_COMMENTS_FAILURE='READ_COMMENTS_FAILURE';
const EDIT_COMMENT='EDIT_COMMENT';
const EDIT_COMMENT_FAILURE='EDIT_COMMENT_FAILURE';
const DELETE_COMMENT='DELETE_COMMENT';
const DELETE_COMMENT_FAILURE='DELETE_COMMENT_FAILURE';
const UNREAD_SUCCESS = 'UNREAD_SUCCESS';
const UNREAD_FAILURE = 'UNREAD_FAILURE';
const CLEAR_SUCCESS = 'CLEAR_SUCCESS';
const CLEAR_FAILURE = 'CLEAR_FAILURE';
const OPTINOUT_SUCCESS = 'OPTINOUT_SUCCESS';
const OPTINOUT_ERRORS = 'OPTINOUT_ERRORS';
const LIKE_COMMENT='LIKE_COMMENT';
const LIKE_COMMENT_FAILURE='LIKE_COMMENT_FAILURE'; 
const LIST_BOOKMARKS='LIST_BOOKMARKS';
const LIST_BOOKMARKS_FAILURE='LIST_BOOKMARKS_FAILURE';
const FETCH_ALL = 'FETCH_ALL';
const FETCH_FAIL = 'FETCH_FAIL';
const GET_HIGHLIGHTS_SUCCESS='GET_HIGHLIGHTS_SUCCESS';
const GET_HIGHLIGHTS_FAILURE=' GET_HIGHLIGHTS_FAILURE';
const REMOVE_HIGHLIGHT_SUCCESS='REMOVE_HIGHLIGHT_SUCCESS';
const CREATE_HIGHLIGHT_SUCCESS='CREATE_HIGHLIGHT_SUCCESS';
const SHOW_HIGHLIGHT_BUTTONS=' SHOW_HIGHLIGHT_BUTTONS';
const CLOSE_HIGHLIGHT_BUTTONS='CLOSE_HIGHLIGHT_BUTTONS';
const SHOW_HIGHLIGHT_MODAL='SHOW_HIGHLIGHT_MODAL';
const CLOSE_HIGHLIGHT_MODAL='CLOSE_HIGHLIGHT_MODAL';
const UPDATE_HIGHLIGH_SUCCESS='UPDATE_HIGHLIGH_SUCCESS';
const UPDATE_HIGHLIGHT_FAILURE='UPDATE_HIGHLIGHT_FAILURE';
const VERIFY_ACCOUNT='VERIFY_ACCOUNT';
const VERIFY_ACCOUNT_FAILURE='VERIFY_ACCOUNT_FAILURE';

const AUTH_DOMAIN = process.env.REACT_APP_FIREBASE_DOMAIN;

firebase.initializeApp({
  apiKey: 'AIzaSyDRQUaRMIlzajWELMP7nDEIy6IMOd6hzeM', 
  authDomain: AUTH_DOMAIN
});

const socialAuth = firebase.auth();
const facebook = new firebase.auth.FacebookAuthProvider();
const google = new firebase.auth.GoogleAuthProvider();
const twitter = new firebase.auth.TwitterAuthProvider();
const FETCH_PROFILE = 'FETCH_PROFILE';
const EDIT_PROFILE = 'EDIT_PROFILE';
const FETCH_PROFILE_ERROR = 'FETCH_PROFILE_ERROR';
const EDIT_PROFILE_ERROR = 'EDIT_PROFILE_ERROR';
const NOTIFICATION_SUCCESS = 'NOTIFICATION_SUCCESS';
const NOTIFICATION_ERROR = 'NOTIFICATION_ERROR';

const LOGOUT = 'LOGOUT';
const BOOKMARK_FAILURE = 'BOOKMARK_FAILURE';
const BOOKMARK_SUCCESS = 'BOOKMARK_SUCCESS';
const UNBOOKMARK_SUCCESS = 'UNBOOKMARK_SUCCESS';

const  ACTION_CONSTANTS = {
  SIGNUP_FAILURE,
  SIGNUP_SUCCESS,
  RESETPASSWORDLINK, 
  RESETPASSWORDLINKERROR,
  PASSWORDCONFIRM,
  PASSWORDCONFIRMERROR,
  LOGIN_FAILURE,
  LOGIN_SUCCESS,
  SOCIAL_FAILURE,
  SOCIAL_SUCCESS,
  SEARCH_SUCCESS,
  SEARCH_FAILURE,
  LIKE_SUCCESS,
  LIKE_FAILURE,
  PROMPT_ACTION,
  PROMPT_CLOSE,
  socialAuth,
  facebook,
  google,
  twitter,
  FETCH_PROFILE,
  EDIT_PROFILE,
  FETCH_PROFILE_ERROR,
  EDIT_PROFILE_ERROR,
  LOGOUT,
  FOLLOWERS_SUCCESS,
  FOLLOWERS_FAILURE,
  FOLLOWING_SUCCESS,
  FOLLOWING_FAILURE,
  FOLLOW_SUCCESS,
  FOLLOW_FAILURE,
  UNFOLLOW_SUCCESS,
  UNFOLLOW_FAILURE,
  CREATE_COMMENT,
  CREATE_REPLY,
  CREATE_COMMENT_FAILURE,
  READ_COMMENTS,
  READ_COMMENTS_FAILURE,
  EDIT_COMMENT,
  EDIT_COMMENT_FAILURE,
  DELETE_COMMENT, 
  DELETE_COMMENT_FAILURE,
  NOTIFICATION_SUCCESS,
  NOTIFICATION_ERROR,
  UNREAD_SUCCESS,
  UNREAD_FAILURE,
  CLEAR_SUCCESS,
  CLEAR_FAILURE,
  OPTINOUT_SUCCESS,
  OPTINOUT_ERRORS,
  LIKE_COMMENT, 
  LIKE_COMMENT_FAILURE,
  LIST_BOOKMARKS,
  LIST_BOOKMARKS_FAILURE,
  FETCH_ALL,
  FETCH_FAIL,
  GET_HIGHLIGHTS_SUCCESS,
  GET_HIGHLIGHTS_FAILURE,
  REMOVE_HIGHLIGHT_SUCCESS,
  CREATE_HIGHLIGHT_SUCCESS,
  SHOW_HIGHLIGHT_BUTTONS,
  CLOSE_HIGHLIGHT_BUTTONS,
  SHOW_HIGHLIGHT_MODAL,
  CLOSE_HIGHLIGHT_MODAL,
  UPDATE_HIGHLIGH_SUCCESS,
  UPDATE_HIGHLIGHT_FAILURE,
  VERIFY_ACCOUNT,
  VERIFY_ACCOUNT_FAILURE,
  BOOKMARK_SUCCESS,
  BOOKMARK_FAILURE,
  UNBOOKMARK_SUCCESS,
};

export default ACTION_CONSTANTS;
